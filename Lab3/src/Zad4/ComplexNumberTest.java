...
class ComplexNumber<T extends Number, U extends Number> implements Comparable<ComplexNumber<?, ?>> {
    ...
    public double modul() {
        // Calculate the modulus using Pythagorean theorem: √(real^2 + imaginary^2)
    ...
    public int compareTo(ComplexNumber<?, ?> other) {
        // Compare the moduli of this complex number and another
        return Double.compare(this.modul(), other.modul());
    }

    @Override
    public String toString() {
        // Format the output with two decimal places for real and imaginary parts
        String sign = imaginary.doubleValue() < 0 ? "-" : "+";
        return String.format("%.2f%s%.2fi", real.doubleValue(), sign, Math.abs(imaginary.doubleValue()));
    }
}
